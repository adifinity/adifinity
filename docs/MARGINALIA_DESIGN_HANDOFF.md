...
- **Date display (Phase 5I-F):** Experience surfaces (register, `/experience/[slug]`, Story annotation, and the Featured-Entry `X` branch) render dates through **`formatExperiencePeriod`** — deliberately coarse **editorial years**, never the stored ISO day precision: ongoing → `2025–present`; no recorded end → the start year (`2024`); a range spanning two calendar years → `2023–2024`; a single-month event → `Jun 2024`; same-year multi-month → the year. Ranges use an **en dash, no spaces**. The stored CMS dates are **never mutated** to achieve this — the coarsening is purely at display. **Work keeps `formatDateRange`** (month precision, em dash) — do not swap Work onto the Experience formatter. Because the coarse period already carries the year(s), the Experience meta lines drop the standalone catalog-year `yearOf` segment (the Archive catalog code still shows `X · YEAR · type` via `catalogLabel`, unchanged).
